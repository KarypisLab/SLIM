...
def load_libslim():
    try:
        ...
        raise RuntimeError("SLIM library %s could not be loaded. Please check if the program is installed correctly.", lib_dir + '/libslim.so')
...
def check_obj_params(params):
    ...
    if hasattr(params, 'dbglvl'):
        if type(params.dbglvl) != int or params.dbglvl < 0:
            raise TypeError("Please select dbglvl from {0, 1, 2, 4, 16, 2048}.")
    else:
        params.dbglvl = 0
    
    if hasattr(params, 'nnbrs'):
        if type(params.nnbrs) != int or params.nnbrs < 0:
            raise TypeError("Please provide non-negative integer value for nnbrs.")
    else:
        params.nnbrs = 0
    
    if hasattr(params, 'simtype'):
        if params.simtype not in slim_simtype_et:
            raise TypeError("Please select simtytpe from {'cos', 'jacc', 'dotp'}.")
    else:
        params.simtype = 'cos'
    ...
    if hasattr(params, 'nthreads'):
        if type(params.nthreads) != int or params.nthreads <= 0:
            raise TypeError("Please provide positive integer value for nthreads.")
    else:
        params.nthreads = 1
    
    if hasattr(params, 'niters'):
        if type(params.niters) != int or params.niters <= 0:
            raise TypeError("Please provide positive integer value for niters.")
    else:
        params.niters = 50
    
    if hasattr(params, 'nrcmds'):
        if type(params.nrcmds) != int or params.nrcmds <= 0:
            raise TypeError("Please provide positive integer value for nrcmds.")
    else:
        params.nrcmds = 10
...
class SLIMatrix(object):
        ...
        if isinstance(data, scipy.sparse.csr.csr_matrix):
            ...
            if oldmat != None and isinstance(oldmat, SLIMatrix):
                if self.nUsers != oldmat.nUsers or self.nItems != oldmat.nItems:
                    raise TypeError("The size of the input matrix does not match the size of oldmat.")
    ...
    def data_from_np2d(self, data, oldmat=None):
        if oldmat != None:
            assert isinstance(oldmat, (SLIMatrix, SLIM)), 'Please feed in a SLIMatrix object or a SLIM model for oldmat.'
        ...
        if miss > 0:
            print("%d of the events fall out of the range of oldmat. Partial entries collected." % (miss))
...
class SLIM(object):
    ...
    def train(self, params, data):
        ...
        if isinstance(params, dict):
            ...
            try:
                ...
                raise TypeError('Parameter type %s is not supported!' % (type(params).__name__))
    ...
    def mselect(self, params, trndata, tstdata, arrayl1, arrayl2, nrcmds):
        ...
        assert type(arrayl1) in [list, np.ndarray], 'Please provide a list of l1 values.'
        assert type(arrayl2) in [list, np.ndarray], 'Please provide a list of l2 values.'
        ...
        if rstatus == SLIM_OK:
            ...
            print('The best HR is achieved by, l1: %.4f, l2:%.4f, HR:%.4f, AR:%.4f.' % (bestl1HR.value, bestl2HR.value, bestHRHR.value, bestARHR.value))
            print('The best AR is achieved by, l1: %.4f, l2:%.4f, HR:%.4f, AR:%.4f.' % (bestl1AR.value, bestl2AR.value, bestHRAR.value, bestARAR.value))
        else:
            raise RuntimeError('Something went wrong with model estimation or evaluation when l1=%.4f, l2=%.4f. Please check the input matrix.' % (bestl1HR, bestl2HR))
        ...
        if rstatus == SLIM_OK:
            ...
            if outfile:
                ...
                for key, value in out.items():
                    f.write(str(key) + ': ' + np.array2string(value, max_line_width=np.inf) + '\n')
        else:
            raise RuntimeError('Something went wrong during prediction. Please check 1) if the model is estimated correctly; 2) if the input matrix for prediction is correct.')
    ...
    def load_model(self, modelfname, mapfname):
        ...
        self.ismodel = self._slim_load(byref(self.handle), c_char_p(modelfname.encode('utf-8')))
